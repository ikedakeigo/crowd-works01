jQuery(function($){
  ...
  themeOptionsForm.on('change', '.qt-btn-preview-wrapper :input', function() {
      ...
      type2 : [color, color, 'transparent'],
		...
		Object.keys(obj).forEach(key => {
			...
			target.attr('data-colors', obj[key]);
		});
		...
		Object.keys(obj).forEach(key => {
			var target = $cl.find( '.qt_button.' + key );
			target.attr('data-hover-colors', obj[key]);
		});
	...
	// マウスオーバー時のアクション
	$(document).on('mouseenter', '.admin_preview_area .qt_button', function(){

		var hoverColors = $(this).attr('data-hover-colors').split(',');
		$(this).css({ 'color' :hoverColors[0], 'borderColor': hoverColors[1] });
		if(!$(this).hasClass('type3')){
			$(this).find('.background').css('backgroundColor', hoverColors[2]);
		}
	});
	$(document).on('mouseleave', '.admin_preview_area .qt_button', function(){
  ...
  $('.full_screen_button').on('click', function() {
	...
	// トップページのボタンプレビュー
	$(document).on('change', '.qt_button_preview_option input[name*="_button_type"]', function(){

		var closest = $(this).closest('.option_list');
		var value = closest.find('input[name*="_button_type"]:checked').val();
		var button = $('#qt_preview_' + value).html();
		var target = closest.next('.admin_preview_area');
	...
	$('.qt_button_preview_option input[name*="_button_type"]').trigger('change');

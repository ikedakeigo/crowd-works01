...
<div class="wrap">
 ...
 <h2><?php _e( 'TCD Theme Options', 'tcd-ankle' ); ?></h2>
 ...
 <div class="updated fade">
  <p><strong><?php _e( 'Updated', 'tcd-ankle' ); ?></strong></p>
 </div>
 <?php endif; ?>
 ...
 <div id="my_theme_option" class="cf">
  ...
  <div id="my_theme_right">
   ...
   <div id="saving_data" style="display:none;"><p><?php _e('Now saving...', 'tcd-ankle'); ?></p></div>
  </div>
...
function theme_options_tools_notices() {
	...
	if ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'error' === $tools_result[1] ) {
		...
		if ( isset( $tools_result[2] ) ) {
			...
			switch( $int_import_error ) {
				...
				case 8:
					...
					break;
				case 11:
                  $message = $int_import_error . ' : ' . __( 'File has not been uploaded.', 'tcd-ankle' );
					break;
				case 12:
                  $message = $int_import_error . ' : ' . __( 'There is no file', 'tcd-ankle' );
					break;
				case 13:
                  $message = $int_import_error . ' : ' . __( 'The file extension is not .json', 'tcd-ankle' );
					break;
				case 14:
                  $message = $int_import_error . ' : ' . __( 'Failed to read the file', 'tcd-ankle' );
					break;
				case 15:
                    $message = $int_import_error . ' : ' . __( 'Json decoding failed', 'tcd-ankle' );
					break;
				default :
		...
		echo '<div class="update-message notice notice-error notice-alt"><p><strong>' . sprintf( __( 'Import error. %s', 'tcd-ankle' ), $message ) . '</strong></p></div>';

	// インポート成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings imported', 'tcd-ankle' ) . '</strong></p></div>';

	// リセット成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'reset' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings reset', 'tcd-ankle' ) . '</strong></p></div>';
	}
}
...
/**
 * ankle オプションTools デフォルト画像設定フィルター
 */
function ankle_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		...
		// スライダー
		array(
			'media_filename' => 'image_1000x600.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1000x600.gif',
			'theme_option_keys' => array("['index_slider'][0]['image']")
		),
		// array(
		// 	'media_filename' => 'image_1450x420.gif',
		// 	'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x420.gif',
		// 	'theme_option_keys' => array("archive_blog_header_bg_image","archive_news_header_bg_image","archive_menu_header_bg_image","archive_gallery_header_bg_image")
		// ),
	);
}
...
/**
 * ankle オプションTools サンプルカテゴリー
 */
function ankle_theme_options_tools_set_sample_categories( $dp_options ) {
...
/**
 * ankle オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function ankle_theme_options_tools_set_sample_posts( $dp_options ) {

	// ブログのアイキャッチ画像
	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_720x495.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_720x495.gif'
	), true );
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample post', 'tcd-ankle' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-ankle' ),
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample news', 'tcd-ankle' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-ankle' ),
...
/**
 * ankle オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function ankle_theme_options_tools_set_sample_menus( $dp_options ) {
	...
	if ( $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach( $nav_menus as $nav_menu ) {
			if ( $nav_menu->name == __( 'Sample menu', 'tcd-ankle' ) )
				return $dp_options;
		}
	}

	// サンプルメニュー作成
	$menu_id = wp_create_nav_menu( __( 'Sample menu', 'tcd-ankle' ) );
...
/**
 * ankle オプションTools サンプルウィジェット -----------------------------------------------------------------
 */
function ankle_theme_options_tools_set_sample_widgets( $dp_options ) {
	...
	$sample_widgets = array(
		...
		// バナー
		array(
			'class_name' => 'tcd_banner_widget',
			'params' => array(
				'banner_title' => '50%OFF SALE',
				'banner_sub_title' => '',
				'banner_font_color' => '#ffffff',
				'banner_url' => '#',
				'banner_image' => 	theme_options_tools_get_media_id( array(
					'media_filename' => 'image_300x250.gif',
					'source_filepath' => get_template_directory() . '/admin/img/default/image_300x250.gif'
				), true ),
				'banner_use_overlay' => 1,
				'banner_overlay_color' => '#961717'
			)
		),
		// 記事スライダー
		array(
 ...
 * ankle テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'ankle_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'ankle_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'ankle_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'ankle_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'ankle_theme_options_tools_set_sample_widgets', 10, 1 );

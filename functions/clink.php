...
function get_internal_clink_html( $id, $atts ) {
  ...
  if ( ! $excerpt ) {
    if ( $post->post_excerpt ) {
      ...
      // 吹き出し対策
      $excerpt = preg_replace('/\[speech_balloon_.*?\]/','', $excerpt);
      $excerpt = preg_replace('/\[\/speech_balloon_.*?\]/','', $excerpt);
      $excerpt = get_the_custom_excerpt( $excerpt, 150 );
    }
  }
  ...
  $clink  = '<div class="cardlink">
    ...
    <div class="content">
     <div class="title_area">
      ...
      <h4 class="title no_editor_style">
       <a href="' . esc_url( $atts['url'] ) . '">' . esc_html( $title ) . '</a>
      </h4>
     </div>
     <p class="desc no_editor_style"><span>' . esc_html( $excerpt ) . '</span></p>
    </div>
   </div>' . "\n";

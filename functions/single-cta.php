...
function tcd_cta_scripts() {
	...
	wp_enqueue_script( 'force-cta', get_template_directory_uri() . '/admin/js/single-cta.js', array( 'jquery' ), version_num(), true );
	wp_localize_script( 'force-cta', 'tcd_cta', array( 'admin_url' => admin_url( 'admin-ajax.php' ), 'ajax_nonce' => wp_create_nonce( 'tcd_cta_nonce' ) ) );

}

if ( '5' !== $options['single_cta_display'] ) {  
...
function tcd_admin_cta_scripts() {
	...
	wp_enqueue_script( 'force-cta', get_template_directory_uri() . '/admin/js/single-cta.js', array( 'jquery' ), version_num(), true );
	wp_localize_script( 'force-cta', 'tcd_cta', array( 'admin_url' => admin_url( 'admin-ajax.php' ), 'ajax_nonce' => wp_create_nonce( 'tcd_cta_nonce' ), 'confirm_text' => __( 'Are you sure to reset these values?', 'tcd-ankle' ) ) );
...
function get_cta_in_random_display() {
	...
	for ( $i = 1; $i <= 3; $i++) {
		if ( $options['single_cta_random' . $i] ) {
...
// 	$options = get_design_plus_option();
// 	if ( ! is_single() || ( is_single () && '5' === $options['single_cta_display'] ) ) return;
